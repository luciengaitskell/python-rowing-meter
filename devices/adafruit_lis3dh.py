...
Taken from Adafruit, modified by Lucien Gaitskell (2018)

* Ported for MicroPython (esp32)
"""
...
class LIS3DH:
    ...
    def __init__(self, int1: Pin=None, int2: Pin=None):
        # Check device ID.
        ...
        self._int2 = int2
        if self._int1:
            self._int1.init(mode=Pin.IN, pull=Pin.PULL_UP)
...
class LIS3DH_I2C(LIS3DH):
    ...
    def __init__(self, i2c: I2C, *, address: int=0x18, int1: Pin=None, int2: Pin=None):
        self._i2c = i2c
        self._addr = address
        super().__init__(int1=int1, int2=int2)

    def _read_register(self, register, length):
        return self._i2c.readfrom_mem(self._addr, register & 0xFF, length)

    def _write_register_byte(self, register, value):
        self._i2c.writeto_mem(self._addr, register & 0xFF, bytes([value & 0xFF]))


class LIS3DH_SPI(LIS3DH):  # TODO: FINISH and VERIFY this
    """Driver for the LIS3DH accelerometer connected over SPI."""
    # Yet to be tested / implemented

    def __init__(self, spi: SPI, cs: Pin, *, baudrate=100000, int1: Pin=None, int2: Pin=None):
        raise NotImplementedError("in-dev: UNTESTED")
        from devices.support import spi_device
